...
use tuix_application::application::{Application, TuixRenderer};

pub struct WgpuRenderer { }

impl TuixRenderer for WgpuRenderer {
	fn run(self, window: Window, event_loop: EventLoop<()>, event_manager: EventManager, state: State) {
		pollster::block_on(self.run_internal(application))
	}
}

impl WgpuRenderer {
	async fn run_internal(self, application: Application) {
